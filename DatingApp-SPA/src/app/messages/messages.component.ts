...
export class MessagesComponent implements OnInit {
  ...
  deleteMessage(id: number) {
    this.alertify.confirmMsg('Are you sure you want to delete this message?', () => {
      this.userService.deleteMessage(id, this.authService.decodedToken.nameid)
        .subscribe(() => {
          this.messages.splice(this.messages.findIndex(m => m.id === id), 1);
          this.alertify.successMsg('Message has been deleted!');
        }, error => {
          this.alertify.errorMsg('Failed to delete the message!');
        });
    });
  }
